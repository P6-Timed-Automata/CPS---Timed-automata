...
def run_ta_pipeline(method_type, params, single_trace_data, tag_k=2):
    ...
    if learner.ta.inconsistency_nb(learner.tss, True) > 0:
        print(f"  WARNING: inconsistent automaton for {method_type} {params}")
        # Either skip this config, or record an 'inconsistent' flag in results
...
def run_variant(label, method_type, params, data_lists, trace_files, t_raws, v_raws, tag_k):
    """
    ...
    for i, trace_data in enumerate(data_lists):
        ...
        # Fit on this trace only — single-trace corpus
        discretized_traces, bins_c = discretize_corpus(method_type, params, [trace_data])

        # Learn TA from this trace only
        learner, n_states, n_edges, is_consistent, n_inconsistencies = \
            learn_ta_from_corpus(discretized_traces, bins_c, tag_k)
        ...
        # MAE computed against this trace's raw signal
        disc = discretized_traces[0]   # only one trace in the corpus
        t_d, v_d = discretized_to_step(disc, bins_c)
    ...
    result = {
        ...
        # MAE across traces — median + range
        ...
        # Structure across traces — median + range
        ...
        # Consistency: count of consistent TAs out of N
        "n_consistent":    int(sum(per_trace_consistent)),
        "n_total":         len(per_trace_consistent),
        ...
        # Plot data (from median-MAE trace)
    ...
    print(
        f"    MAE={result['mae_median']:.3f} (median, range {result['mae_min']:.3f}-{result['mae_max']:.3f}) | "
        f"time={result['time_median']:.3f}+/-{result['time_std']:.3f}s | "
        f"states={result['n_states_median']:.1f} ({result['n_states_min']}-{result['n_states_max']}) | "
