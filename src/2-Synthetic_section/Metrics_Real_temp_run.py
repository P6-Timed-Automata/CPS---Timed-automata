...
Same parameter choices as exp_51_52 final config — single configuration per
method, no sweep (Path A: minimum-effort real-data extension).

Companion: exp_54_plot.py reads results.json and produces figures/tables.
...
# extend with METHOD_VARIANTS dict like exp_51_52_run.py.
METHODS = [
    ("naive",   {"bins": 10}),
    ("sax",     {"w": 48,  "bins": 8}),   # w tuned for 288-sample 24h traces
    ("persist", {"bins": 8}),
]

# ---- Data paths -------------------------------------------------------------
TRAIN_FOLDER    = ROOT / "Data" / "3-ExtractInterval" / "1day-experiment" / "roomA-train"
TEST_POS_FOLDER = ROOT / "Data" / "3-ExtractInterval" / "1day-experiment" / "roomA-test"
...
# Number of negatives per anomaly mode (4 modes total → 4 × this many)
N_NEG_PER_MODE = 15
...
PHASE_SHIFT_HOURS     = 6      # half-period
...
def _inject_stuck(trace, duration_s):
    ...
    stuck_value = float(temps[midpoint_idx])
    temps[in_stuck] = stuck_value
    return (times, temps)


def _inject_phase_shift(trace, shift_hours):
    """
    Shift the time series in time. Equivalent to rotating the trace by
    shift_hours of samples. This makes day-night patterns occur at
    "wrong" times of day.
    """
    times, temps = trace
    times = np.asarray(times)
    ...
    # Find the index closest to shift_seconds into the trace
    shift_idx = np.searchsorted(times - times[0], shift_seconds)
    shift_idx = int(shift_idx) % len(temps)
    if shift_idx == 0:
        return (times, temps)
...
def build_injected_negatives(test_pos, n_per_mode, seed=42):
    """
    Take real test traces and apply each of the four anomaly modes to
    a subset of them. Returns (neg_traces, neg_modes) matching the
    interface generate_negative_set() uses.

    The same n_per_mode test traces are perturbed for each mode (sampled
    with replacement if necessary). Different modes pick different traces
    to avoid one outlier trace dominating multiple modes.
    """
    rng = np.random.default_rng(seed)
    if len(test_pos) == 0:
    ...
    # Mode 3: phase shift (called "shifted" in NEG_MODE_NAMES)
    for _ in range(n_per_mode):
        idx = int(rng.integers(0, len(test_pos)))
...
def save_config(out_dir, train_traces, test_pos, neg_traces, neg_modes):
    ...
    if NEGATIVE_MODE == "inject":
        lines += [
            "",
            "--- Injection parameters (inject mode) ---",
...
if __name__ == "__main__":
    ...
    if not train_traces:
        print("ERROR: No training traces found.")
        sys.exit(1)
    if not test_pos:
        print("ERROR: No positive test traces found.")
        sys.exit(1)

    # ---- Build negatives ----------------------------------------------------
    if NEGATIVE_MODE == "inject":
        neg_traces, neg_modes = build_injected_negatives(test_pos, N_NEG_PER_MODE, seed=42)
    ...
    for method, params in METHODS:
        ...
        # Save incrementally so a crash mid-method preserves partial results
