"""
Metrics_scaling_plot.py
=======================
Reads results.json from Metrics_scaling_run.py and renders:

  f1_vs_n.png         — F1 (median + min/max band) vs corpus size
  recall_vs_n.png     — same for recall
  precision_vs_n.png  — same for precision
  per_mode_heatmap_<method>.png — rows = N, cols = anomaly modes

Run with no arguments to auto-select the latest log.
"""
...
def find_latest_log():
    base = ROOT / "Data" / "Graphs" / "Metrics_scaling"
    if not base.is_dir():
        return None
    candidates = []
...
def _extract_method_series(log, method_name, metric):
    """For one method, return (n_train_levels, medians, mins, maxs) for a metric."""
    n_levels, medians, mins, maxs = [], [], [], []
    for entry in log["results"]:
        m = entry["methods"].get(method_name)
        if m is None or not _is_plottable(m):
            continue
        overall = m.get("overall", {})
        if metric not in overall:
            continue
        n_levels.append(entry["n_train"])
        medians.append(overall[metric]["median"])
        mins.append(overall[metric]["min"])
        maxs.append(overall[metric]["max"])
    return n_levels, medians, mins, maxs
...
def plot_metric_vs_n(log, metric, ylabel, title, out_path):
    fig, ax = plt.subplots(figsize=(10, 5))

    methods = _all_method_names(log)
    for method in methods:
        n_levels, medians, mins, maxs = _extract_method_series(log, method, metric)
        if not n_levels:
            continue
        color = METHOD_COLORS.get(method, "gray")
        ax.fill_between(n_levels, mins, maxs, color=color, alpha=0.18,
                        label=f"{method} (min-max)")
        ax.plot(n_levels, medians, marker="o", linewidth=2, markersize=5,
                color=color, label=f"{method} (median)")

    ax.set_xlabel("Training-corpus size (N)")
    ax.set_ylabel(ylabel)
    n_repeats = log.get("n_repeats", "?")
    ax.set_title(f"{title}  (median across {n_repeats} repeats, shaded = min-max)")
...
def plot_per_mode_heatmap_per_method(log, out_dir):
    ...
    methods    = _all_method_names(log)
    n_levels   = [e["n_train"] for e in log["results"]]

    for method in methods:
        data     = np.full((len(n_levels), len(mode_names)), np.nan)
        statuses = ["unknown"] * len(n_levels)
        ...
        ax.set_yticklabels([str(n) for n in n_levels])
        ax.set_ylabel("Training-corpus size (N)")
...
def main():
    ...
    if args.log is None:
        ...
        if args.log is None:
            print("No results.json found. Run Metrics_scaling_run.py first.")
            return
    ...
    print(f"Repeats per cell:  {log.get('n_repeats', '?')}")
    print(f"Output folder:     {out_dir}\n")

    print("=== Plots ===")
    plot_metric_vs_n(log, "f1",        "F1",        "F1 vs training-corpus size",
                     out_dir / "f1_vs_n.png")
    plot_metric_vs_n(log, "recall",    "Recall",    "Recall vs training-corpus size",
                     out_dir / "recall_vs_n.png")
    plot_metric_vs_n(log, "precision", "Precision", "Precision vs training-corpus size",
                     out_dir / "precision_vs_n.png")
