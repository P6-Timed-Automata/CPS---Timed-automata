...
def _extract_method_series(log, method_name, metric):
    ...
    for entry in log["results"]:
        ...
        if metric not in m["overall"]:
            continue
        noise_levels.append(entry["noise_std"])
        medians.append(m["overall"][metric]["median"])
        mins.append(m["overall"][metric]["min"])
        maxs.append(m["overall"][metric]["max"])

    return noise_levels, medians, mins, maxs


def _all_method_names(log):
    return [m["method"] for m in log["methods"]]
...
def plot_metric_vs_noise(log, metric, ylabel, title, out_path):
    ...
    for method in methods:
        noise_levels, medians, mins, maxs = _extract_method_series(log, method, metric)
        if not noise_levels:
            continue
    ...
    # F1 threshold reference line (only meaningful for F1)
    ...
    # Clean / noisy reference noise levels
    for level_key, label_text in [("clean_noise", "clean"),
                                  ("noisy_noise", "noisy")]:
...
def plot_per_mode_heatmap_per_method(log, out_dir):
    """
    One heatmap per method: rows = noise levels, columns = anomaly modes,
    cells = median rejection rate across seeds.
    """
...
def _find_threshold_crossing(noise_levels, medians, threshold):
    """
    Find the first noise level where F1 median drops below threshold.
    Also returns the last noise level where it was above.
    """
    crossing_at = None
    last_above = None
...
def write_threshold_summary(log, out_dir):
    ...
    for method in methods:
        noise_levels, medians, mins, maxs = _extract_method_series(log, method, "f1")
        crossing, last_above = _find_threshold_crossing(noise_levels, medians, threshold)
        ...
        if crossing is None:
            ...
            last_above_str = f"{last_above:.3f}" if last_above is not None else "n/a"
        ...
        rows.append({
            "method":          method,
            "f1_at_clean":     f"{f1_at_clean:.3f}" if f1_at_clean else "n/a",
            "f1_at_noisy":     f"{f1_at_noisy:.3f}" if f1_at_noisy else "n/a",
            "drops_below_at":  crossing_str,
            "last_above":      last_above_str,
        })

        print(
            f"  {method:8s} | F1@clean={rows[-1]['f1_at_clean']} | "
            f"F1@noisy={rows[-1]['f1_at_noisy']} | "
            f"drops at {crossing_str} (last above: {last_above_str})"
        )
...
def main():
    ...
    print("=== Plots ===")
    plot_metric_vs_noise(log, "f1", "F1", "F1 vs training noise",
                         out_dir / "f1_vs_noise.png")
    plot_metric_vs_noise(log, "recall", "Recall", "Recall vs training noise",
                         out_dir / "recall_vs_noise.png")
    plot_metric_vs_noise(log, "precision", "Precision",
                         "Precision vs training noise",
                         out_dir / "precision_vs_noise.png")

    # --- Per-mode heatmaps per method ---------------------------------------
