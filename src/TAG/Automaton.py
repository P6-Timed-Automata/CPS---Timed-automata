...
class Automaton:
    ...
    def export_ta(self, path: str, symbol_map: dict = None, time: int = 86400, sim_nr: int = 1) -> None:
        ...
        # ----------------------------
        # Reset transitions from final states to initial state
        # ----------------------------
        ...
        for state in self.states:
            ...
            if state.accepting:
                ...
                lines.append('    <transition>')
                lines.append(f'      <source ref="{state_ids[state.name]}"/>')
                lines.append(f'      <target ref="{state_ids[initial.name]}"/>')

                # Optional: make reset always enabled
                lines.append(
