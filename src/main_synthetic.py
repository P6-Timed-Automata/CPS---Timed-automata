...
# Paths
experiment_folder = BASE_DIR / "Data" / "synthetic_data" / "clean"

discretinize_data_path = (
    BASE_DIR / "Data" / "4-DiscretizationData" / discretization_method / period
    / f"{room}-{period}-{discretization_method}-s{symbols}.txt"
)
...
rawTraces  = all_traces  # use all synthetic traces (only 20)
...
for k in range(k_min, k_max + 1, k_increment):
    ...
    title = f"{room}-{period}-{discretization_method}-s{symbols}-k{k}"

    xml_path = (
        BASE_DIR / "Data" / "6-XMLOutput" / discretization_method / period
        / f"{room}-{period}-{discretization_method}-s{symbols}-k{k}.xml"
    )
