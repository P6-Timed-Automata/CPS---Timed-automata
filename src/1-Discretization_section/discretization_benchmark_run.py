...
Results are written to a timestamped subfolder under TA_Benchmark/ so runs never
...
def run_experiment(method_name, t_raw, v_raw, variants_dict, data_lists, trace_files, tag_k=2):
    ...
    for label, (method_type, params) in variants_dict.items():
        ...
        result = {
            ...
            "params":        params,
            "rmse_mean":     float(np.mean(per_trace_rmse)),
            "rmse_std":      float(np.std(per_trace_rmse)),
...
if __name__ == "__main__":
    ...
    print("\n=== Persist ===")
    persist_vars = {
        f"bins_max={b}": ("persist", {"bins": b})
        for b in [3, 5, 7, 9, 11, 13, 15, 17]
    }
