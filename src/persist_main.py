...
period = sys.argv[1] if len(sys.argv) > 1 else "ecg"
...
experiment_folder = resolve_experiment_folder(BASE_DIR, period)
...
for trace_nr in range(start_traces, len_traces):
    ...
    p = Persist(x=ts, break_min=break_min, break_max=break_max,
                divergence="w", candidates="EW", skip=np.array([skip_min, skip_max]))
    ...
    save_path = (BASE_DIR / "Data" / "Graphs" / "persistGraph" /
                 f"{room}-{trace_nr}trace-{period}-s{symbols}-skipmin{skip_min}-skipmax{skip_max}-breakpoints.png")
    ...
    for k in range(k_min, k_max + 1, k_increment):
        ...
        TA_output_path = (BASE_DIR / "Data" / "5-TaResults" /
                          discretization_method / period)
        ...
        print(
            f"Done: trace={trace_nr}, k={k}, symbols={symbols},skipmin{skip_min},skipmax{skip_max}-")
